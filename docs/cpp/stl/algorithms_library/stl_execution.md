...
[cppreference \<execution\>](https://en.cppreference.com/w/cpp/header/execution)
...
- `std::par` -> Parallel policy
    ...
    - you should use `std::mutex`, `std::atomic<T>`
...
- `std::unseq` -> Unsequenced policy
    ...
    - you can't use `std::mutex::lock()` because of `interleaved`. It can cause deadlock.
