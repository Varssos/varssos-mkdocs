...
!!! important
    For `remove()` and `unique()` you have to erase elements on the back of the container!!
...
!!! important
    `stable_partition()` keeps order of input elements, in contrast to `partition()`
...
!!! important
    The `[first, last)` range has to be sorted with the less operator or with a comparison function
