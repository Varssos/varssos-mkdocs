...
(If this method is unavailable to you, it is also possible to compile gdbserver using the SDK and install it manually. For that purpose, make sure to go to menuconfig (in SDK) and select Development ---> gdbserver.)
...
To stop `gdbserver` on target invoke on host's `remote-gdb` session command:
```bash
monitor exit
```
