...
[MQTT in details on YT](https://www.youtube.com/watch?v=TSwgZn2FKUw&list=PLS1QulWo1RIbQWue3zzwEuV2pSASiDxvv&index=1)
...
- Perfect fit for constrained devices with limited resources
...
- **MQTT client** is any device (from a micro controller up to a fully-fledged server) that runs an MQTT library (e.g. libmosquito) and connects to an MQTT broker over network
