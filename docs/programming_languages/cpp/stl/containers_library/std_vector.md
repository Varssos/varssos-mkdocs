...
!!! important
    If possible, try to reserve enough memory with `vec.reserve(<size>);`. Every time you push_back an element to a vector and its capacity is exceeded, it has to reserve contiguous memory with enough memory (it depends, but in most cases n->2n) and copy all data to the new location.

!!! important
    It is preferred to use `vec.emplace_back( args )` instead of `vector::push_back( args )`. It creates an object in place and avoids additional copying.
...
**Or you can use the Erase-Remove Idiom**:
...
## Iterate over the elements in a vector and do an action only for elements which fulfill a condition

The problem is that a simple `std::find_if()` returns only one iterator for the first element.
