...
2. Install all necessary packages for cinnamon with lightdm:
```bash
sudo apt install lightdm-settings slick-greeter
```

3. You can verify configs here: `/etc/lightdm/...`
4. If cinnamon is not installed, install like [here](https://tecadmin.net/install-cinnamon-on-ubuntu/)
5. On start you should check if cinnamon is selected if the ubuntu UI is presented like in the link above
...
There was a problem that in network manager there weren't any wired connections. After `ifconfig enp2s0f0 up` there was still a problem.
...
Solution: [Deprecated linux apt-key](https://opensource.com/article/22/9/deprecated-linux-apt-key)

Alternatively you can:

1. Erase the gpg file from here: `/usr/share/keyrings` e.g. `brave-browser-archive-keyring.gpg`
2. Remove the source list from `/etc/apt/sources.list.d` e.g. `brave-browser-release.list`
3. Install the app once again from scratch

## Black monitor on display connected to hdmi

Reason:

1. hdmi output is connected only to the nvidia GPU on some laptops

Solution:

1. You have to install nvidia drivers
2. Switch to `Nvidia (Performance Mode)`
3. You can update the kernel
...
On ubuntu: Settings->Device Colour Profiles->Choose e.g. sRGB or whatever different profile works for you
