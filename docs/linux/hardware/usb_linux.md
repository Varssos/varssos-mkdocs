...
1. Look for the physical usb address e.g. `usb 1-7.2.3` in `sudo dmesg` in `usb 1-7.2.3: FTDI USB Serial Device converter now attached to ttyUSB1`

Where:

- `1-7.2.3` is the physical usb address. Bus: 01 and Ports: 7.2.3. You can compare with `lsusb -tv`. It should remain constant if you don't physically switch the USB port.
- `ttyUSB1` is the usb port to which you want to connect, e.g. `picocom -b 115200 /dev/ttyUSB1`
...
2. Disconnect usb port (just insert the physical address into the command below)
```bash
...
Let's assume that variable `path` has the output of the above command.
...
4. After connecting this device, not only `/dev/ttyUSB1` should be visible but also `/dev/qnx_serial` as a symbolic link.
