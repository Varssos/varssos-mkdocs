...
[Academind git & github 2h course](https://www.youtube.com/watch?v=ulQA5tjJark&t=6555s)
...
A cloud git repository & services provider. It stores & manages git repositories.
...
- `git init`. This creates a `.git` folder in the current folder and inits local repo
- You can create repo in gitlab or github in web interface

### Create Commits

1. `git add <files(s)>` Stage changes for next commit
2. `git commit` Create a commit that includes all staged changes

```bash
git add index.html
git commit -m "Initial commit"
git log #(optionally). It prints only commits before and current HEAD
```

### Move between Commits

- `git checkout <id>` Temporarily move to another commit. It changes pointer `HEAD` to desired commit id
...
!!! warning
    Be careful that this will create an "alternate reality" for people who have already fetched/pulled/cloned from the remote repository. But in fact, it's quite simple:
...
In the root folder you can add a `.gitignore` file and in each line specify files/directories which should be ignored by `git add`
...
Rebase erases branch history and aligns to one branch
...
- `git fetch` - download commits from remote origin and make them available locally
...
### Save your credentials to avoid asking for credentials each time on git push/pull
...
1. `Settings` -> `Collaborators` and then add collaborator
